...
| 提供 AccessKey（密钥） | 产品经理 | 相当于把"云操作权"交给 AI，用完可随时删除 |
...
| 存数据 | 轻量方案：预留实例 + 定期云备份（SQLite）；正规方案：云数据库 RDS PostgreSQL | 数据量、并发上去后升级 RDS |
| 登录 | 邀请码登录 | 已有公司 SSO 时对接 |
| 监控 | 结构化日志 + trace_id + Sentry | 已有公司监控平台时接入 |

**注意**：火山引擎的 RDS PostgreSQL **只对企业实名认证用户开放**。个人认证账号用不了。因此本手册把"预留实例常驻 + 定期备份到对象存储 + 启动自动恢复"作为默认轻量方案，它不需要企业认证、费用低、足够早期使用；业务做大后再升级 RDS。
...
2. **实名认证**：个人认证即可完成大部分部署；但**云数据库 PostgreSQL 只对企业认证开放**。如果走默认轻量方案（预留实例 + 云备份），个人认证即可，无需企业认证。
...
| 云数据库 RDS | RDS / 数据库 | 存数据（轻量方案可暂不开，但建议开） |
...
**AI 做（运行时版本）**：veFaaS 默认运行环境是 **native-python3.12**（本地若是 Python 3.11 也没关系，但部署前要确认代码兼容 3.12，避免上线才发现语法/依赖不兼容）。
...
| 云数据库 PostgreSQL 提示"只对企业用户开放" | RDS PostgreSQL 需企业实名认证 | 个人认证账号改用"预留实例 + 云备份"轻量方案，或完成企业认证 |
...
| 部署成功后数据隔段时间就丢 | `/tmp` 是临时存储，实例缩容被回收 | `--minInstance 1` 预留常驻实例 + 定期云备份 + 启动恢复 |
...
- [ ] 确认需要产品经理决策的事项（登录方式、预算、数据留存规则），用产品语言列出问题，不要技术名词

## 云平台准备（产品经理操作，AI 给清单）
- [ ] 确认账号状态：主账号还是子账号（用密钥调身份接口确认）
- [ ] 确认实名认证状态（个人/企业；是否影响所选方案）
- [ ] 拿到 AccessKey（AK/SK）
...
- [ ] 实现登录与数据隔离（邀请码换 token、接口按用户过滤）
- [ ] 实现数据持久化（预留实例 + 定期备份到对象存储 + 启动自动恢复）
...
- 先给我一份"上线方案摘要"，用产品语言说明：要开哪些云服务、大概费用量级、需要我配合做哪几步、会得到什么结果；
...
- 部署完成后，给我访问地址、登录方式（邀请码）和一份我可照做的验收清单；
