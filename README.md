...
首次使用时，在注册页填写邮箱、密码和邀请码，获取并输入邮箱验证码，完成邮箱绑定与注册。随后按提示填写昵称、选择性别并完成位置授权，即可开始使用。

注册完成后，使用 **邮箱和密码登录**，无需重复输入邀请码。如果此前已经使用邀请码建立了穿搭档案，也需要在注册页用原邀请码绑定邮箱，原有资料会保留。

### 允许访问位置和天气

首次完善资料时，请勾选页面中的 **「允许访问位置和天气」**，并在浏览器弹出位置访问请求时选择 **「允许」**。WearCue 会根据当前位置获取当地天气，用于生成当天的穿搭推荐。

这里需要授权的是浏览器的位置访问，天气会根据位置自动获取，无需单独开启“天气权限”。如果之前拒绝了定位，请在浏览器的网站权限设置中重新允许位置访问，再返回页面重试。
...
- 查看所在城市的全天体感温度范围、降水概率、阵风与紫外线信息，以及防雨、防晒提醒。
- 直接查看整套搭配的单品和薄厚，减少从头组合的纠结。
- 想试试别的风格，点击「换一套」，也可以进入方案查看具体穿法。

![今日穿搭，根据天气与通勤、约会、出行场景推荐整套搭配](docs/readme-images/today-outfit.png)

### 个人穿搭灵感

把自己的穿搭照或喜欢的搭配图片上传，AI 会识别服装款式、颜色与薄厚，并给出适用温度范围。确认或调整识别结果后，就能保存整套穿搭，并加入首页推荐，在体感温度和场景匹配时再次用上。

![照片识别入口，选择一张穿搭照片并上传识别](docs/readme-images/photo-recognition.png)

### AI 穿搭建议

看到一套喜欢的穿搭后，还需要知道怎么穿。WearCue 通过 AI 生成穿搭名称、搭配分析与具体步骤，并为系统推荐生成穿搭效果图，将示例照片、单品清单和搭配说明放在一起。

- 对照示例图查看衣物、鞋子与配饰，逐步完成搭配。
- 了解版型和层次如何组合，按体感温度增减衣物。
- 没有同款单品时，参考替代建议，用相近的款式或颜色搭配。

![穿搭方案，以高山机能徒步为例展示单品、穿搭步骤、温度调整与替代建议](docs/readme-images/outfit-guide.png)
...
- 收藏想保留的搭配，删除不再需要的穿搭。
- 将个人穿搭加入首页推荐，在当天体感温度与场景匹配时优先推荐。
- 不希望某套穿搭继续出现在首页时，可以将它移出推荐，仍保留在穿搭库中。
...
设置每天的提醒时间，在出门前及时查看当天穿搭。可以根据自己的作息调整时间，也可以随时关闭提醒。
