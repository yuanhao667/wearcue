...
> **定位先说清**：本套手册共三份，是给 **AI 的执行 + 引导剧本**，不是给产品经理的教程。AI 读完三份，负责把其中"产品经理操作"的部分，翻译成一步一步的可执行指令（去哪个网站、点哪个、填什么、看什么、确认哪句）。产品经理只需要照着做 + 确认 + 验收，不需要理解任何技术、设计、部署概念。
...
```text
开始前  AI 先检查产品经理电脑环境（Python / Node / Git 是否就绪，缺什么 AI 引导装什么）；
        模型 Key 有没有——没有的话 AI 告诉产品经理去哪申请，开发期可先用 mock，验收前补齐
第 0 步  PM：把「PRD + 三份手册」交给 AI
第 1 步  AI 读本手册 → 输出《技术适配声明》
...
>
> 三份手册里都自带"直接交给 AI 的固定指令"（通用手册第十三/十四节、前端手册第 25 节、部署手册附录），AI 会按当前阶段自动选用对应指令；产品经理不需要自己挑选或拼接指令。
...
把一份新 PRD 交给 AI 开始开发前，产品经理需要用产品语言说明以下问题。**不要求产品经理设计数据库或 API；缺失的技术表达由 AI 在技术适配声明和阶段文档中补全。**如果缺项会显著改变用户体验、成本、安全或交付范围，先询问；其余缺项可以采用手册默认值并显式记录。
...
| 层 | 默认方案 | 规则层级 | 简单理解 |
| --- | --- | --- | --- |
| 后端语言 | Python 3.11.x | B 默认方案 | 编写业务、Agent 和 AI 能力 |
...
## 6. 需要产品经理决定的问题
- 只列会改变产品目标、成本、安全或交付范围的问题；没有则写“无”
```
...
2. **运行环境**：确认项目所需解释器的真实版本和路径。新建默认 Python 项目可用 Python 3.11.x 创建 `.venv`；现有项目优先遵循已有版本约束，不擅自升级。
...
├── docs/                 # PRD、本手册和分阶段技术开发文档的副本
├── .env.example
├── .gitignore
└── README.md
```

`docs/` 中存放 PRD、手册、阶段文档和技术适配声明的副本，保证 AI 在项目目录内就能拿到决策上下文。
...
| 能力 | 本地开发 / MVP | 正式生产环境 |
| --- | --- | --- |
| 结构化数据 | SQLite 或带 schema 的结构化文件 | 根据关系、并发和查询需求迁移到 PostgreSQL 或公司数据平台 |
| 向量检索 | 本地 Chroma | pgvector 或公司指定的向量服务 |
| 文件存储 | 本地目录 | 对象存储 |
| 后台任务 | 后端执行并持久化状态 | 达到并发与可靠性阈值后使用任务队列或公司任务平台 |
| 登录 | 基础登录或暂不实现 | OAuth、企业 SSO 或公司统一身份系统 |
| 部署 | 本地运行 | Docker、云服务器或公司容器平台 |
...
- Python 3.11.x（新建默认项目；现有项目按已有约束，开工时验证真实路径与版本）
- Git
...
1. 把 PRD、本手册、已有代码和已知限制交给 AI。
2. 对照第二节检查七类产品信息；技术缺项由 AI 推导，只有重大产品缺项才询问。
