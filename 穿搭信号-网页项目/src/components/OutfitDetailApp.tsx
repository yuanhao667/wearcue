...
export function OutfitDetailApp({ id }: { id: string }) {
  ...
  return <main className="paper-page outfit-detail-page">
    ...
    <div className={`outfit-detail-layout${showPhotoColumn ? "" : " single"}`}>
      {showPhotoColumn && <section className="outfit-detail-photo-card"><div className="card-caption">{originalImageUrl ? savedOutfit?.source === "system" ? "穿搭示例照片" : "穿搭照片" : "AI 穿搭效果图"}</div>{imageUrl ? <figure className="outfit-detail-photo"><img src={imageUrl} alt={`${label}穿搭参考`} />{(savedOutfit?.source === "system" || generatedImageUrl) && <figcaption>例图由AI生成</figcaption>}</figure> : <div className="outfit-detail-photo-placeholder" role="status" aria-live="polite">{imageLoading ? <><b aria-hidden="true"><span className="recognition-dots"><i /><i /><i /></span></b><h3>正在生成穿搭效果图</h3><p>AI 正在还原人物、场景与整套衣物</p></> : <p className="cross-audience-notice" role="alert"><span className="cross-audience-notice-icon" aria-hidden="true">!</span><span className="cross-audience-notice-copy">{imageError}</span></p>}</div>}</section>}
