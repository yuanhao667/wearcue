...
- SQLite 本地数据持久化；配置 `PERSISTENT_DATA_DIR` 后自动备份、启动恢复，并把图片写入持久化卷；
...
AI 模型按任务分流：视觉识别使用 `VISION_MODEL`；首页实时换一套固定以 `AI_FAST_MODEL` 作为首选且唯一模型，当前默认 `qwen-turbo`，只返回紧凑的图标映射与后续生图输入，关闭思考模式，`max_tokens=480`，并由 `AI_REALTIME_TIMEOUT_SECONDS`（默认 30 秒，上限 60 秒）限制整条调用，禁止切换到 `AI_QUALITY_MODEL`；AI 命名与详情建议使用 `AI_FAST_MODEL`；系统推荐预生成使用 `AI_QUALITY_MODEL`。人物图片只在用户进入详情页且无缓存图时生成，不在“换一套”阶段提前生成。当前生产依赖 qwen3.8-flash、qwen-turbo 与 `AI_IMAGE_MODEL`。AIHubMix 主域名在本地网络不可达时，使用其官方同能力备用接口 `https://api.inferera.com/v1`。
...
域名可以绑定到 veFaaS/APIG 的 HTTP 入口，但域名本身不保存数据。生产环境使用 `/tmp` 运行 SQLite，并通过 `PERSISTENT_DATA_DIR=/mnt/tos` 将一致性数据库快照、用户上传图和 AI 生成图保存到 veFaaS 的 TOS 读写挂载目录；实例缩容后可在下次启动时恢复。生产配置通过部署环境和平台健康检查确认，不再对外暴露内部配置状态。
